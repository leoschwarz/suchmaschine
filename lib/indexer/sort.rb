...
module Indexer
  class IndexFileChunk
    ...
    def initialize(index_file, chunk_index)
    ...
    def load
      @rows = @index_file.read_bin_entries(@chunk_index*MAX_ROWS, MAX_ROWS)
    end
  ...
  class IndexSorter
    ...
    def self.sort(unsorted_file, destination_file)
      ...
      if chunks == 1
        ...
        temp_file = Common::IndexFile.new(self.new_temp_file_path)
        (0...chunks).each do |chunk_i| # TODO Später evtl einen speziellen Enumerator verwenden.
          chunk = IndexFileChunk.new(temp_file, chunk_i)
          chunk.load
    ...
    # source_file: Common::IndexFile
    # output_file: Common::IndexFile
    # chunks_n: Integer, Anzahl Stücke in der Datei. Jeweils MAX_ROWS*ROW_SIZE gross, das letzte ist unter Umständen aber kleiner.
    # chunk_range: Range, nil: Falls Range ein spezieller Indexbereich für den IndexFileChunks berücksichtigt werden sollen.
    ...
    def self.merge_sort(source_file, destination_file, chunks_n, chunk_range=nil)
      ...
      if chunks_n > MAX_CHUNKS
        ...
        # Deshalb werden [MAX_CHUNKS] viele IndexFileChunkList Objekte erzeugt.
        ...
        # Rekursion
        sorted_subfiles = (0...MAX_CHUNKS).map{ Common::IndexFile.new(self.new_temp_file_path) }
        ...
        # die Chunks in eine IndexFileChunkList geladen, die sicherstellt dass immer nur ein Chunk geladen ist.
        chunk_lists = []
        sorted_subfiles.each_with_index do |subfile, index|
          chunks = (0...sorted_subfiles_chunks_n[index]).map{|i| IndexFileChunk.new(subfile, i)}
          chunk_lists << IndexFileChunkList.new(chunks)
        end
        
        result = IndexFileBufferedWriter.new(destination_file)
        ...
        chunks = chunk_range.map{|i| IndexFileChunk.new(source_file, i)}
