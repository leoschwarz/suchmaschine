module Crawler
  class RobotsTXTCacheItem
    ...
    def save
      Crawler::Database.cache_set("robotstxt:#{self.domain}", self.serialize) if Config.robotstxt.cache.enabled
    end

    # Sucht nach einem robots.txt Cache-Eintrag für eine bestimmte Domain.
    def self.load(domain)
      item = self.deserialize(Crawler::Database.cache_get("robotstxt:#{domain}")) if Config.robotstxt.cache.enabled
